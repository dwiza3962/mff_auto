...
NEMU_EXE = "NemuPlayer.exe"
NEMU_KEY_HANDLE_NAME = "NemuPlayer"
NEMU_KEY_HANDLE_CLASS = "Qt5QWindowToolSaveBits"
NEMU_CHILD_NAME = "Canvas"
...
class MuMuPlayer(AndroidEmulator):
    """Class for working with NoxPlayer emulator."""

    def __init__(self, name="MuMuPlayer", child_name="ScreenBoardClassWindow", key_handle_name=None):
    ...
    def _set_params_by_version(self):
        ...
        self.key_handle_name = NEMU_KEY_HANDLE_NAME
        self.key_handle_class = NEMU_KEY_HANDLE_CLASS
        self.child_name = NEMU_CHILD_NAME
    ...
    def _read_config_file(config_file="conf.ini"):
        ...
        :rtype: ConfigParser
        """
        path = os.path.join(os.getenv('APPDATA'), "..", "Local", "Nemu", config_file)
        try:
            config = ConfigParser()
            config.read(path)
            return config
    ...
    def _get_virtual_machine_id(self):
        """Gets Nemu's Virtual Machine ID of current process.
        ...
        try:
            ...
            return logging.warning(f"Can't find Numu's multiplayer settings in %APPDATA% folder by path: {path}.")
    ...
    def _get_keyboard_shortcut_for_closing_app(self):
        ...
        #                         f"Try to open Nemu settings -> `Interface settings` and then change position "
